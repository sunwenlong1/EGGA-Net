...
class EdgeAttention(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        # Sobel 算子卷积核
        self.sobel_x = nn.Conv3d(1, 1, kernel_size=3, padding=1, bias=False)
        self.sobel_y = nn.Conv3d(1, 1, kernel_size=3, padding=1, bias=False)
    ...
    def init_sobel_kernel(self):
        ...
        self.sobel_x.weight.data = sobel_x_kernel.unsqueeze(0)
        self.sobel_y.weight.data = sobel_y_kernel.unsqueeze(0)
        self.sobel_x.weight.requires_grad = False  # Sobel 核权重固定
        self.sobel_y.weight.requires_grad = False

    def forward(self, x):
        # Sobel 边缘提取（仅对第一通道进行边缘计算）
        edge_x = F.conv3d(x[:, :1, :, :, :], self.sobel_x.weight, padding=1)
        edge_y = F.conv3d(x[:, :1, :, :, :], self.sobel_y.weight, padding=1)

        # 计算梯度幅值（添加 epsilon 避免 NaN）
        edge_features = torch.sqrt(edge_x ** 2 + edge_y ** 2 + 1e-8)

        # 调整边缘特征的尺寸与通道数
        edge_features = F.interpolate(edge_features, size=x.shape[2:], mode='trilinear', align_corners=False)
        if x.size(1) > 1:  # 若 x 的通道数大于 1，则重复边缘特征到相同通道数
            edge_features = edge_features.repeat(1, x.size(1), 1, 1, 1)
...
class EdgeTrackingModule(nn.Module):
    ...
    def forward(self, x):
        ...
        edge_x = F.conv3d(x[:, :1, :, :, :], self.sobel_x.weight, padding=1)
        edge_y = F.conv3d(x[:, :1, :, :, :], self.sobel_y.weight, padding=1)
        edge_features = torch.sqrt(edge_x ** 2 + edge_y ** 2 + 1e-8)  # 添加 epsilon 防止 NaN
...
class GAM_Attention(nn.Module):
    def __init__(self, planes, rate=4):
        ...
        self.spatial_attention = nn.Sequential(
            nn.Conv3d(planes, int(planes / rate), kernel_size=3, padding=1),
            nn.BatchNorm3d(int(planes / rate)),
            nn.ReLU(inplace=True),
            nn.Conv3d(int(planes / rate), planes, kernel_size=5, padding=2),
            nn.BatchNorm3d(planes),
            nn.Conv3d(planes, planes, kernel_size=7, padding=3),
            nn.BatchNorm3d(planes)
        )
