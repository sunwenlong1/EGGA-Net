...
class EdgeAttention(nn.Module):
    ...
    def init_sobel_kernel(self):
        ...
        self.sobel_x.weight.requires_grad = False  # 固定 Sobel 核权重
        self.sobel_y.weight.requires_grad = False

    def forward(self, x):
        # Sobel 边缘提取（仅对第一个通道进行操作）
        edge_x = F.conv2d(x[:, :1, :, :], self.sobel_x.weight, padding=1)
        edge_y = F.conv2d(x[:, :1, :, :], self.sobel_y.weight, padding=1)

        # 计算梯度幅值
        edge_features = torch.sqrt(edge_x ** 2 + edge_y ** 2 + 1e-8)

        # 调整边缘特征的尺寸与通道数
        if x.size(1) > 1:
            edge_features = edge_features.repeat(1, x.size(1), 1, 1)
        ...
        # 将边缘特征与注意力机制融合
        enhanced_attention = sigmoid * edge_features

        # 输出加权结果
        return x * enhanced_attention
...
class EdgeTrackingModule(nn.Module):
    ...
    def init_sobel_kernel(self):
        ...
        self.sobel_x.weight.data = sobel_x_kernel
        self.sobel_y.weight.data = sobel_y_kernel
        self.sobel_x.weight.requires_grad = False  # Sobel 核权重固定
        self.sobel_y.weight.requires_grad = False

    def forward(self, x):
        # Sobel 边缘提取（仅对第一通道进行边缘计算）
        edge_x = F.conv2d(x[:, :1, :, :], self.sobel_x.weight, padding=1)
        edge_y = F.conv2d(x[:, :1, :, :], self.sobel_y.weight, padding=1)

        # 计算梯度幅值（添加 epsilon 避免 NaN）
        edge_features = torch.sqrt(edge_x ** 2 + edge_y ** 2 + 1e-8)

        # 调整边缘特征尺寸与通道数
        edge_features = F.interpolate(edge_features, size=x.shape[2:], mode='bilinear', align_corners=False)
        if x.size(1) > 1:  # 若 x 的通道数大于 1，则重复边缘特征到相同通道数
            edge_features = edge_features.repeat(1, x.size(1), 1, 1)

        # 主干卷积操作
        residual = self.adjust_channels(x)  # 使用1x1卷积调整通道数
        x = self.conv1(x)
        x = x + edge_features  # 融合边缘特征
        x = self.conv2(x)
        x = self.conv3(x)
        x = x + residual  # 添加残差连接
...
class cbam(nn.Module):
    def __init__(self, planes):
        ...
        self.ea = EdgeAttention(planes, planes)
...
class GAM_Attention(nn.Module):
    def __init__(self, planes, rate=4):
        ...
        reduced_channels = max(planes // rate, 8)  # 确保最低通道数为 8

        # 通道注意力模块
        self.channel_attention = nn.Sequential(
            nn.Linear(planes, reduced_channels),
            nn.ReLU(inplace=True),
            nn.Linear(reduced_channels, planes),
            nn.LayerNorm(planes)  # 添加标准化层提高稳定性
        )
        self.channel_att = channel_att(planes)
        # 多尺度空间注意力模块
        self.spatial_attention = MultiScaleSpatialAttention(planes, rate)
        # 位置注意力
        self.local_att = local_att(planes)
    ...
    def forward(self, x):
        ...
        x_permute = x.flatten(2).permute(0, 2, 1)  # 调整形状为 [B, H*W, C]
        x_channel_att = self.channel_attention(x_permute).permute(0, 2, 1).view(b, c, h, w)
        ...
        x_local_att = local_att(out)

        out_All = out * x_local_att
...
class MultiScaleSpatialAttention(nn.Module):
    def __init__(self, planes, rate=4):
        ...
        reduced_channels = max(planes // rate, 8)

        # 多尺度卷积
        self.conv1 = nn.Conv2d(planes, reduced_channels, kernel_size=3, padding=1)
        self.conv2 = nn.Conv2d(planes, reduced_channels, kernel_size=5, padding=2)
        self.conv3 = nn.Conv2d(planes, reduced_channels, kernel_size=7, padding=3)
        self.fuse = nn.Conv2d(3 * reduced_channels, planes, kernel_size=1)

    def forward(self, x):
        x1 = self.conv1(x)
        x2 = self.conv2(x)
        x3 = self.conv3(x)
        fused = torch.cat([x1, x2, x3], dim=1)  # 在通道维度上融合
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.ca = ChannelAttention(planes)  # 添加通道注意力模块
        self.sa = SpatialAttention()  # 添加空间注意力模块
...
class Decoder(nn.Module):
    def __init__(self, params):
        ...
        self.up4_2 = UpBlock2(self.ft_chns[1], self.ft_chns[0], self.ft_chns[0], dropout_p=0.0)
        self.out_conv = nn.Conv2d(self.ft_chns[0], self.n_class, kernel_size=3, padding=1)
        # 添加 CBAM 模块到解码器的每个 BasicBlock 中
        # self.cbam_blocks = nn.ModuleList([cbam(self.ft_chns[i]) for i in range(len(self.ft_chns))])
        ...
        self.edge_attention = EdgeAttention(self.ft_chns[4], self.ft_chns[4])
        self.x0_edge_attention = EdgeTrackingModule(self.ft_chns[0], self.ft_chns[0])
        self.x1_edge_attention = EdgeAttention(self.ft_chns[1], self.ft_chns[1])
        self.x1_edg_conv = nn.Conv2d(self.ft_chns[2], self.ft_chns[1], kernel_size=3, padding=1)
        self.x0_edg_conv = nn.Conv2d(self.ft_chns[1], self.ft_chns[0], kernel_size=3, padding=1)
    ...
    def forward(self, feature):
        ...
        # x2 = self.cbam3(x2) + x2
        # x3 = self.cbam4(x3) + x3
        x1_edg = self.x0_edge_attention(x1)
        x2_edg = self.x1_edge_attention(x2)
        # x4 = self.edge_attention(x4)

        x5 = self.cbam5(x5)
        x5_up = self.up1(x5, x4)

        x6 = self.cbam4(x5_up) + x5_up
        x6_up = self.up2(x6, x3)

        x7 = self.cbam3(x6_up) + x6_up
        x7_up = self.up3(x7, x2)

        x7_up = torch.cat([x7_up, x2_edg], dim=1)
        x7_up = self.x1_edg_conv(x7_up)
        x8 = self.cbam2(x7_up) + x7_up

        x8_up = self.up4(x8, x1)

        x8_up = torch.cat([x8_up, x1_edg], dim=1)
        x8_up = self.x0_edg_conv(x8_up)
        x8_up = self.cbam1(x8_up) + x8_up

        # 在每个 BasicBlock 后应用 CBAM 模块
        # for i, cbam_block in enumerate(self.cbam_blocks):
        #     x_last = cbam_block(x_last)

        # x_last = self.edge_attention(x_last)

        output = self.out_conv(x8_up)
        return output, x8_up
